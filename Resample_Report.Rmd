...
The following scatterplot (`r fig_num("f_1km_1km_QGISaggr_NDVI_eur_scat", display = "cite")`) represents the correlation between the resampled map values using the two tools presented in this assessment. It shows a very high correlation between them, reporting a correlation coefficient (Pearson's *r*) of `r comp_results[3, 2]` (`r table_num("comp_results_table", display = "cite")`). Additionally, RMSE and MAE were `r comp_results[3, 3]` and `r comp_results[3, 4]`, respectively.


![`r fig_1km_1km_R_QGISaggr_NDVI_eur_scat`](`r orig_R_QGISaggr_NDVI_eur_scat`)


In light of the two assessments made on both the R-based and the QGIS/Python-based tools, as well as the results showed in this subsection, it can be confirmed that both tools can be equally used, only based on user's preferences.
