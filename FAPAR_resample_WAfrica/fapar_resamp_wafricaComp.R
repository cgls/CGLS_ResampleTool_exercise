...
if(Sys.info()[4] == "D01RI1700371"){
  ...
  path2save <- "E:/rotllxa/NDVI_resample/NDVI_resample_WAfrica"
}else if(Sys.info()[4] == "h05-wad.ies.jrc.it"){
  path2data <- ""
  path2save <- ""
}else if(Sys.info()[4] == "MacBook-MacBook-Pro-de-Xavier.local"){
  path2data <- "/Users/xavi_rp/Documents/D6_LPD/NDVI_data"
  path2save <- "/Users/xavi_rp/Documents/D6_LPD/NDVI_resample/FAPAR_resample_WAfrica"
}else{
...
#cropping to WAfrica
fapar_1km_orig_WAfr <- crop(fapar_1km_orig, my_extent)
as.vector(extent(my_extent))
as.vector(extent(fapar_1km_orig_WAfr))
summary(getValues(fapar_1km_orig_WAfr))

jpeg(paste0(path2save, "/fapar_1km_orig_WAfr.jpg"))
plot(fapar_1km_orig_WAfr)
dev.off()

fapar1km_rstr <- fapar_1km_orig_WAfr
...
#cropping to WAfrica
fapar_300m_orig_WAfr <- crop(fapar_300m_orig, my_extent)
as.vector(extent(my_extent))
as.vector(extent(fapar_300m_orig_WAfr))
summary(getValues(fapar_300m_orig_WAfr))

jpeg(paste0(path2save, "/fapar_300m_orig_WAfr.jpg"))
plot(fapar_300m_orig_WAfr)
dev.off()


if(all(round(extent(fapar_300m_orig_WAfr)[1], 7) %in% round(x_ext, 7) &
       round(extent(fapar_300m_orig_WAfr)[2], 7) %in% round(x_ext, 7) &
       round(extent(fapar_300m_orig_WAfr)[3], 7) %in% round(y_ext, 7) &
       round(extent(fapar_300m_orig_WAfr)[4], 7) %in% round(y_ext, 7))){
...
jpeg(paste0(path2save, "/fapar1km_300m_WAfr.jpg"),
...
save(list = stuff2save, file = paste0(path2save, "/ResampleResults_fapar_wafrica_4Report.RData"))
#load(paste0(path2save, "/ResampleResults_fapar_wafrica_4Report.RData"), verbose = TRUE)




## Comparison 'original-1km' with '300m-resampled-1km-QGIS_Aggr' ####
qgis_resamp_wafrica_avrge <- paste0(path2data, "/QGIS_CGLT_20180810/fapar2018.tif")
...
rsmpl_df <- data.frame(getValues(fapar1km_rstr), getValues(qgis_resamp_wafrica_avrge))
rsmpl_df <- rsmpl_df[complete.cases(rsmpl_df), 1:2]
...
xyplot(rsmpl_df_subsample$getValues.qgis_resamp_wafrica_avrge. ~ rsmpl_df_subsample$getValues.fapar1km_rstr., 
       type = c("p", "r"),
       col.line = "red",
       xlab = "1km original FAPAR product",
...
# Calculating differences (errors)
rsmpl_df$diff <- abs(rsmpl_df$getValues.fapar1km_rstr. - rsmpl_df$getValues.qgis_resamp_wafrica_avrge.)
rsmpl_df$diff1 <- abs(round(rsmpl_df$getValues.fapar1km_rstr., 1) - round(rsmpl_df$getValues.qgis_resamp_wafrica_avrge., 1))
...
par(mfrow = c(1, 2), mar = c(4, 4, 4, 5))
plot(fapar1km_rstr, main = "FAPAR 1km (original)")
plot(qgis_resamp_wafrica_avrge, main = "FAPAR 1km (resampled)") 
dev.off()
