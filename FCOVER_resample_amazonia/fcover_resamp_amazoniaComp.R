...
if(date == "may19"){
  ...
  #load(paste0(path2save, "/ResampleResults_fcover_amazonia_4Report_May19.RData"), verbose = TRUE)
}else if(date == "august18"){
...
qgis_resamp_amazonia_avrge <- paste0(path2data, "/QGIS_CGLT/fcover.tif")
...
rsmpl_df <- data.frame(getValues(fcover1km_rstr), getValues(qgis_resamp_amazonia_avrge))
rsmpl_df <- rsmpl_df[complete.cases(rsmpl_df), 1:2]
...
xyplot(rsmpl_df_subsample$getValues.qgis_resamp_amazonia_avrge. ~ rsmpl_df_subsample$getValues.fcover1km_rstr., 
       type = c("p", "r"),
       col.line = "red",
       xlab = "1km original FCOVER product",
...
# Calculating differences (errors)
rsmpl_df$diff <- abs(rsmpl_df$getValues.fcover1km_rstr. - rsmpl_df$getValues.qgis_resamp_amazonia_avrge.)
rsmpl_df$diff1 <- abs(round(rsmpl_df$getValues.fcover1km_rstr., 1) - round(rsmpl_df$getValues.qgis_resamp_amazonia_avrge., 1))
...
par(mfrow = c(1, 2), mar = c(4, 4, 4, 5))
plot(fcover1km_rstr, main = "FCOVER 1km (original)")
plot(qgis_resamp_amazonia_avrge, main = "FCOVER 1km (resampled)") 
dev.off()
